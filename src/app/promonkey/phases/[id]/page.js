...
function TaskRow({ task, teamOptions, onPatch, onDelete, onOpen }) {
    ...
    return (
        <div className="flex items-center gap-3 py-2.5 border-b border-[#EEF1F6] last:border-0 flex-wrap">
            ...
            {/* Title / meta — click opens the task detail page at /promonkey/task/[id] */}
            <button
                onClick={() => onOpen(task.id)}
                className="min-w-0 flex-1 text-left"
                title="Open task"
            >
                <p className={cn("text-[13.5px] font-semibold hover:text-[#3C80F5] transition-colors", task.status === "DONE" ? "text-[#94A3B5] line-through" : "text-[#1B2330]")}>
                    {task.title}
                </p>
...
/* ---------------- Sub-phase group (collapsible) ---------------- */

function SubPhaseGroup({ subPhase, teamOptions, onPatchTask, onAddTask, onDeleteTask, onEditSubPhase, onDeleteSubPhase, onOpenTask }) {
            ...
            {open && (
                <div className="pl-6 pb-2">
                    {subPhase.tasks.map(task => (
                        <TaskRow key={task.id} task={task} teamOptions={teamOptions} onPatch={onPatchTask} onDelete={onDeleteTask} onOpen={onOpenTask} />
                    ))}
                    <button
...
export default function PhaseDetailPage() {
    ...
    // Load project (for name, breadcrumb, and member list for assignee dropdowns)
    useEffect(() => {
        ...
        async function load() {
            try {
                ...
                // Non-fatal — phase data already loaded independently.
            }
        }
        load();
    ...
    // Navigates to the standalone task detail page at /promonkey/task/[id],
    // carrying projectId along so that page's Back button can return here.
                    ...
                    {phase.tasks.length === 0 && phase.subPhases.length === 0 ? (
                        ...
                        <div>
                            ...
                            {phase.tasks.map(task => (
                                <TaskRow key={task.id} task={task} teamOptions={teamOptions} onPatch={patchTask} onDelete={deleteTask} onOpen={openTaskDetail} />
                            ))}
